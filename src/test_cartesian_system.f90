...
use, non_intrinsic :: constants_m, only: rk, stdout, g0, ft2nmi, rad2deg
...
use, non_intrinsic :: vector_math_m, only: vmag, heading
...
implicit none
    ...
    do i=1,n
        ...
        write(stdout,'(a,3e14.6,a,f5.2,a,f6.1)') '           Velocity [ft/sec]: ',test(i)%v,',    Speed [mach]: ', &
                                                 vmag(test(i)%v)/mach1(test(i)%x(3)),', Heading [deg]: ', &
                                                 heading(test(i)%v(1:2))*rad2deg
